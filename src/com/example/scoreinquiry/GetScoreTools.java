...
public class GetScoreTools {
	...
	public void parse(){
		...
		for(Element courseE:coursesE){
		  ...
		  course.put("id", childs.get(0).text());	
		  course.put("name", childs.get(1).text());
		  course.put("type", childs.get(2).text());
		  course.put("credit", childs.get(3).text());
		  course.put("teacher", childs.get(4).text());
		  course.put("academy", childs.get(5).text());
		  course.put("major", childs.get(6).text());
		  course.put("year", childs.get(7).text());
		  course.put("term", childs.get(8).text());
		  course.put("score", childs.get(9).text());
	...
	public static List<String> getTerms(List<Map<String,String>> courseInfo,Boolean rawTerm){
		...
		if(rawTerm) {
			...
			for(String term:termSet){
				if(term.charAt(4)=='1') termList.add(term.substring(0,4)+"年上学期");
				else termList.add(term.substring(0,4)+"年下学期");
			}
			return termList;
		}
	}
